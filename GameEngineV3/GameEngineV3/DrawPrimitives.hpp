...
public:
  ...
  void draw(){
    Vector2 size = Application.window()->get_size();
...
public:
  ...
  void refreshPolygon(){
    ...
    Application.renderer()->setRenderColor(lineColor);
    for (int i = 0 ; i < edges.size(); ++i){
      edges[i].draw();
    }
    
    Application.renderer()->setRenderColor(oldColor);
  ...
  void fillPolygon(){
    ...
    int minX =  2555555;
    int minY =  2555555;
    Application.renderer()->setRenderColor(filler.color1);
        ...
        if (inside){
          if (filler.gradient){
            ...
            if (distance < filler.distance){
              ...
              Application.renderer()->setRenderColor(nc);
            }
            else {
              Application.renderer()->setRenderColor(filler.color2);
            }
...
//
    ...
    cout << p->transform.scale.toStr() << endl;
